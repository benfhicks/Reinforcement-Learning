...
class Game:
    ...
    def _check_diagonals(self) -> int:
        ...
        if (
            len(
                set(
                    [
                        self.board[i][len(self.board) - 1 - i]
                        for i in range(len(self.board))
                    ]
                )
            )
    ...
    def _board_to_tensor(self, board):
        ...
        return torch.tensor(np.asarray(board, dtype="float32").reshape(-1, 9)).to(
            device
        )
    ...
    def _best_move(self, player: int, model=None):
        ...
        if model is None:
            ...
            for move in possible_moves:
                ...
                win_prediction = (
                    model_output[0][1].item() if player == 1 else model_output[2]
                )
                lose_prediction = (
                    model_output[0][2].item() if player == 1 else model_output[1]
                )

                move_scores.append(
                    win_prediction
                    if win_prediction > draw_prediction
